...
namespace insoulforge::LlmClient {
    ...
    drogon::Task<std::optional<json>> requestChat(std::string label, std::string usageRole,
      const LLMApiConfig &apiConfig, const LLMModelParams &params, json messages, json tools = {}, u64 sessionId = 0);
    ...
    drogon::Task<std::optional<std::string>> requestLLM(json messages, f64 temperature = 1.35, f64 top_p = 0.92,
      i32 max_tokens = 1024, std::string role = "memory", std::optional<u64> sessionId = std::nullopt,
      f64 timeoutSeconds = 180.0);
    ...
    drogon::Task<std::optional<std::vector<f32>>> requestEmbedding(
      std::string text, std::optional<u64> sessionId = std::nullopt);
} // namespace insoulforge::LlmClient
