...
namespace insoulforge {
    ...
    public:
        ...
        [[nodiscard]] std::optional<json> getSessionMessages(u64 sessionId);
    ...
    private:
        ...
        /// @brief 获取或创建会话工作流状态
        [[nodiscard]] std::shared_ptr<SessionWorkflowState> getOrCreateSessionState(u64 sessionId);

        /// @brief 处理一个会话的消息预处理队列，空队列时立即退出
        drogon::Task<> processPreparationQueue(u64 sessionId);

        /// @brief 处理一个会话的串行回复工作流，无待处理回复时立即退出
        /// @param triggerMessageId 首轮回复的触发入站消息 ID
        drogon::Task<> processReplyWorkflow(u64 sessionId, std::string triggerMessageId);
    };
} // namespace insoulforge
