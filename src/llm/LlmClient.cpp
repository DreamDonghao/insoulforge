...
namespace insoulforge {
    namespace {
        ...
        constexpr i32 kMaxRetries = 3;

        /// @brief 重试间隔
        constexpr std::chrono::seconds kRetryDelay{1};

        /// @brief LLM 请求超时（秒）
        constexpr f64 kLlmTimeoutSeconds = 90.0;
            ...
            if (!respJson) {
                if (sessionId) {
                    Logger::error(*sessionId, "LLM",
                      fmt::format("请求出错: status={}", static_cast<i32>((*resp)->getStatusCode())));
                } else {
                    Logger::error(
                      0, "LLM", fmt::format("请求出错: status={}", static_cast<i32>((*resp)->getStatusCode())));
                }
                co_return std::nullopt;
            }
      ...
      std::string text, const std::optional<u64> sessionId) {
        ...
        if (!requestOk || !data.is_array() || data.empty()) {
            if (sessionId) {
                ...
                Logger::error(
                  0, "Embedding", fmt::format("请求出错: status={}", static_cast<i32>((*resp)->getStatusCode())));
            }
            co_return std::nullopt;
        }
        ...
        for (const auto &value: atOrNull(data[0], "embedding")) {
            embedding.push_back(static_cast<f32>(jsonToDouble(value)));
        }
        if (embedding.empty()) {
            co_return std::nullopt;
        }
    ...
    void LlmClient::logUsage(
      const json &responseJson, const std::string &model, const std::string &role, const std::optional<u64> sessionId) {
        if (!responseJson.contains("usage"))
            return;
        const json &usage = responseJson["usage"];
        i32 promptTokens = getInt(usage, "prompt_tokens");
        i32 completionTokens = getInt(usage, "completion_tokens");
        i32 totalTokens = getInt(usage, "total_tokens");
        ...
        if (usage.contains("prompt_tokens_details")) {
            ...
            const i32 hitTokens = getInt(usage, "prompt_cache_hit_tokens");
            const i32 missTokens = getInt(usage, "prompt_cache_miss_tokens");
        ...
        const u64 logSessionId = sessionId.value_or(0);
        if (promptTokens > 0) {
            f32 hitRate = static_cast<f32>(cachedTokens) / static_cast<f32>(promptTokens) * 100.0f;
